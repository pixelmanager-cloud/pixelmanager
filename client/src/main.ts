...
class Game {
  ...
  private renderTacticsGlossary(lineHigh: boolean) {
    ...
    items.push(`<div class="th-item"><b>Offside Trap</b>${lineHigh ? '' : ' <span class="th-cur">— needs a High or Very High line to do anything</span>'}<br>The back line steps up together as the ball is played through: a striker without a real pace edge on your last defender gets caught. Mistime it — and roughly a third of the time you will — and he is through with your whole line committed.</div>`);
  ...
  private renderLineupEditor() {
    ...
    tac.push(`<label class="tac-toggle" title="${lineHigh ? "Catches a striker who has no real pace edge on your last man — but a mistimed step-up sends him clear." : 'Only bites with a High or Very High defensive line — the back four needs room to step up together.'}"><span>Offside Trap${lineHigh ? '' : ' (needs high line)'}</span><input type="checkbox" id="e-offside" ${this.draftTactics.offsideTrap ? 'checked' : ''} /></label>`);
