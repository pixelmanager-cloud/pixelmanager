...
| F-150 | open | match/content | Live match commentary keys player attributes by NAME, so ~78% of fixtures print at least one epithet earned by a different man | `client/src/main.ts:5868` |
| F-151 | decision | match/content | `pressConferenceLine`'s PRE_CUP pool — 57 authored lines — is unreachable: nothing in the game ever passes `competition: 'cup'` | `shared/src/press.ts:146` |
| F-152 | open | match/content | The PRE-match presser scores stakes from squad strength alone, so a Continental Cup tie is handed the 'routine fixture' value the post-match presser refuses to give it | `client/src/main.ts:5483` |
| F-153 | open | match/content | `goalLine`'s brace/hat-trick tally is keyed by name across both teams, so it announces braces and hat-tricks that did not happen | `client/src/main.ts:6303` |
| F-154 | open | match/content | The commentary corpus probe documents an orphan-key report it does not contain — the check cannot fail | `tools/playtest/commentary_corpus.ts:20` |
| F-155 | open | dynasty/content | The one-son succession copy is unreachable — `showHeirChoice` is only called when siblings exist, so `all.length === 1` can never be true | `client/src/main.ts:3423` |
| F-156 | open | dynasty/content | The Academy list bills every bought outsider as "first of the line" — `pedigreeText` is called without `isFounder`, the one argument that exists to stop this | `client/src/main.ts:3760` |
| F-157 | open | dynasty/content | Two of the three call-up banks (222 of ~305 lines) are unreachable — `careerHonours` hard-codes `scored: 0` and the cap count can never be 1 | `shared/src/tokens.ts:583, branching in shared/src/intl.ts:254-259` |
| F-158 | open | dynasty/content | `capLine` is never folded onto the legend card, so the international sentence vanishes at the succession and no forebear on the Family Record can ever show it | `client/src/api.ts:741` |
| F-159 | open | dynasty/content | Three `.star` manager banks (54 lines) have no reachable emit site — the bloodline player's arrival, sale and retirement never narrate as the bloodline player | `client/src/main.ts:2071, 2089 and 2128` |
| F-160 | open | dynasty/content | Passed-over brothers render as blank medallions on the Family Record, the exact outcome `branchCareer` was written to prevent | `client/src/main.ts:4483, 4523` |
| F-161 | open | career/determinism | Both cross-build replay gates contain an unreachable `st.phase === 'lifestyle'` branch, so no recorded career ever exercises `buyLifestyle` | `shared/qa_replay_contract.ts:53 and tools/playtest/golden_replay.ts:48` |
| F-162 | decision | career/determinism | `careerState`'s `clubLevel` argument has no producer anywhere, so `firstTeamReady`'s "a higher-level club is harder to break into" threshold is permanently 9 | `shared/src/tokens.ts:371` |
| F-163 | open | career/determinism | The side-focus round is keyed on the chapter about to START while the main focus beside it is keyed on the chapter that just ENDED — the exact bug F-103 fixed for the main pick | `shared/src/career.ts:1270` |
| F-164 | open | save/performance | The `missions` collection has no delete path anywhere in the tree, while every reader is scoped to the current season — so scouting rows accumulate in the save for the life of the dynasty, each carrying a full serialized Player | `client/src/save.ts:447` |
