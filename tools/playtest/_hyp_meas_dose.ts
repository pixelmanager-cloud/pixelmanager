...
// DOSE-RESPONSE: hold the DEEPEST FW anchor fixed at 62 (4-4-2's, on the hypothesis's "outside the 30m
// circle" side of its claimed x=69 boundary) and walk the SECOND striker backwards. The hypothesis says
// nothing should move. The rival (the run dies to a legal forward pass) says isolation is what matters.
for (const back of [62, 56, 50, 44, 38]) {
  F[`P${back}`] = FORMATIONS['4-4-2'].map((sl) => ({ ...sl }));
  const a = F[`P${back}`]; a[10] = { ...a[10], x: back };   // slot 10 is the second FW
}
const TESTS = ['P62', 'P56', 'P50', 'P44', 'P38'] as any[];
...
E.pickPassTarget = function (this: any, teamIdx: number, playerIdx: number, goal: any) {
  ...
  if (cur && inFlagged === teamIdx && playerIdx === 9) {
...
E.actCarrier = function (this: any, ...a: any[]) {
  ...
  const mine = cur && t === side && i === 9;   // ONLY the striker whose anchor never moves (slot 9, x=62)
  shotFired = false;
  if (flagged && mine) {
    inFlagged = t;
