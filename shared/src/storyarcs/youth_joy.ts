...
export const YOUTH_JOY_ARCS: StoryArc[] = [
  {
    ...
    beats: {
      ...
      use: {
        ...
        choices: [
          { id: 'try', label: 'Try it', desc: 'In a real game, with people watching', outcome: 'It comes off. Nobody on the touchline knows where it came from except him, and he does not tell anyone, ever.', effect: { form: 0.08, attr: { flair: 1 }, meters: { peers: 5 } } },
          { id: 'simple', label: 'Play the simple pass', desc: 'Keep it for the garden a while longer', outcome: 'He plays it square and files the moment away. It will keep. Most of the good things he learns, he learns to be patient with first.', effect: { attr: { composure: 1, teamwork: 1 }, meters: { authority: 4 } } },
        ],
      },
    },
  },
  {
    ...
    beats: {
      open: {
        ...
        choices: [
          { id: 'idol', label: 'The player he loves', desc: 'SADIKU, 10 — the obvious one, and he does not care', outcome: 'SADIKU 10. He wears it until the collar goes soft, and years later he can still feel exactly which letters had started to peel.', effect: { form: 0.05, attr: { flair: 1 }, tag: 'shirt-idol' } },
          { id: 'own', label: 'His own name', desc: 'Which feels either brave or ridiculous', outcome: 'He asks for his own surname and hears himself say it too quietly, then again louder. He walks out of the shop feeling like he has told a small lie he now has to make true.', effect: { attr: { leadership: 1, composure: 1 }, meters: { family: 4 }, tag: 'shirt-own-name' } },
          { id: 'blank', label: 'Leave it blank', desc: 'Just the shirt, no claim on it', outcome: 'He keeps it plain, which nobody comments on, and which he decides privately is the coolest option available to a boy with eleven pounds.', effect: { attr: { composure: 1 }, tag: 'shirt-blank' } },
        ],
      },
    },
  },
  {
    ...
    beats: {
      open: {
        ...
        choices: [
          { id: 'weak', label: 'Only the wrong foot, all summer', desc: 'A hundred a day, left foot only', outcome: 'Six weeks of it. The wall does not congratulate him. By August the left one is merely bad instead of useless, and that gap is worth years.', effect: { energy: -6, attr: { stamina: 1, composure: 1 }, form: 0.04, tag: 'wall-weak-foot' } },
  ...
  },
  {
    ...
    beats: {
      open: {
        ...
        choices: [
          { id: 'loud', label: 'Do it properly, out loud', desc: 'Full commentary, name and all', outcome: 'He says his own name in the third person and it makes him laugh, and then he says it again seriously. The imagined stadium is very detailed. He knows exactly where the away end is.', effect: { form: 0.06, attr: { flair: 1, creativity: 1 }, tag: 'commentates' }, next: 'heard' },
          { id: 'quiet', label: 'Keep it inside his head', desc: 'Where nobody can hear it', outcome: 'He mouths it silently, which somehow makes it more real, not less. It becomes a habit he will still have at twenty-five, before kick-off, without noticing.', effect: { attr: { composure: 2 }, tag: 'inner-commentary' }, next: 'heard' },
        ],
      },
      heard: {
        id: 'heard',
        ...
        choices: [
          { id: 'own-it', label: 'Laugh and carry on doing it', desc: 'It costs nothing to be caught enjoying something', outcome: 'He laughs harder than anyone and is back out there the next evening, narrating. Not being embarrassed by what you love is a small superpower and he has just been handed it.', effect: { form: 0.05, meters: { family: 6 }, attr: { flair: 1 } } },
          { id: 'stop', label: 'Stop doing it out loud', desc: 'Move the whole stadium indoors', outcome: 'The commentary goes silent and lives behind his eyes instead. It never actually stops. It just gets a better sound system.', effect: { attr: { composure: 1 }, meters: { family: 2 } } },
        ],
      },
    },
  },
  ...
  },
  {
    ...
    beats: {
      open: {
        ...
        choices: [
          { id: 'private', label: 'Keep the quiet one', desc: 'The one with no witnesses', outcome: 'He says the bin-bag one out loud for the first time and it sounds like nothing. He keeps it anyway. Some things are load-bearing precisely because nobody else can see them.', effect: { attr: { composure: 2 }, form: 0.04, tag: 'memory-private' } },
          { id: 'shared', label: 'Tell the famous one', desc: 'The story the family already owns', outcome: 'He tells the one everyone knows the ending of, and enjoys the way the room joins in halfway through. He has just learned that a good story is a thing you share out.', effect: { meters: { family: 6 }, attr: { leadership: 1 }, form: 0.03, tag: 'memory-shared' } },
        ],
      },
    },
  },
  {
    ...
    beats: {
      open: {
        ...
        choices: [
          { id: 'nine', label: 'Take the nine', desc: 'The number that comes with a job description', outcome: 'He takes the nine and immediately feels obliged to score, which is either a burden or an engine depending on the week. Mostly an engine.', effect: { form: 0.05, attr: { leadership: 1, aggression: 1 }, tag: 'number-nine' } },
          { id: 'four', label: 'Take the four', desc: 'Nobody grows up dreaming of the four', outcome: 'He takes the four because somebody has to, and finds he quite likes being the one the game goes through rather than the one it ends at.', effect: { attr: { teamwork: 2, composure: 1 }, meters: { authority: 5 }, tag: 'number-four' } },
          { id: 'odd', label: 'Take the twenty-three with the hole', desc: 'For no reason he can explain', outcome: 'He takes the broken one because he likes the number, and wears it for three seasons out of pure stubbornness until it means something simply by having lasted.', effect: { attr: { flair: 1, composure: 1 }, meters: { peers: 4 }, tag: 'number-23' } },
        ],
      },
    },
  },
  {
    ...
    beats: {
      open: {
        ...
        choices: [
          { id: 'keep', label: 'Keep the whole ritual', desc: 'It costs ninety seconds and buys calm', outcome: 'He keeps it. It is not magic and he half knows that; what it actually does is tell his hands the game is starting, and his hands are grateful for the warning.', effect: { attr: { composure: 2 }, form: 0.05, tag: 'ritual-kept' }, next: 'broken' },
          { id: 'test', label: 'Deliberately break it once', desc: 'To prove he is not superstitious', outcome: 'He puts the right boot on first, on purpose, staring at it. He plays fine. He is slightly disappointed by how fine he plays.', effect: { attr: { composure: 1, leadership: 1 }, tag: 'ritual-tested' }, next: 'broken' },
        ],
      },
      broken: {
        id: 'broken',
        ...
        choices: [
          { id: 'improvise', label: 'Invent a smaller one on the spot', desc: 'Three touches of the ball will do', outcome: 'He taps the ball three times with the outside of his boot before kick-off and decides that counts. It counts. It has always been him doing it, not the boots.', effect: { attr: { composure: 2, creativity: 1 }, form: 0.05 } },
    ...
    beats: {
      open: {
        ...
        choices: [
          ...
          { id: 'watch', label: 'Stay at the back and watch him', desc: 'How he is with the small kids', outcome: 'He hangs back and watches how the man handles a nervous six-year-old, and notices that the kindness is the effortful bit, not the football.', effect: { attr: { leadership: 2, teamwork: 1 }, tag: 'hero-watched' }, next: 'after' },
        ],
      },
      after: {
        id: 'after',
  ...
  },
  {
    ...
    beats: {
      open: {
        ...
        choices: [
          { id: 'keep', label: 'Hide it at the back of the drawer', desc: 'Nobody needs to know', outcome: 'He folds it small and buries it under everything else. He will still have it when he is thirty and will not be able to explain it convincingly to anyone.', effect: { attr: { composure: 1 }, meters: { family: 3 }, form: 0.03, tag: 'shirt-kept' } },
          { id: 'give', label: 'Put it in the bag', desc: 'Let a smaller kid have a good shirt', outcome: 'He puts it in and closes the bag quickly. Somewhere in the town a seven-year-old is now wearing his best summer, and he finds he does not mind that at all.', effect: { attr: { teamwork: 2 }, meters: { family: 5 }, tag: 'shirt-given' } },
          { id: 'frame', label: 'Pin it to the bedroom wall', desc: 'Where he can see it from the bed', outcome: 'Four drawing pins and a slightly crooked hang. It is the first thing he sees every morning for two years, and it is not nostalgia — it is a to-do list.', effect: { attr: { leadership: 1, composure: 1 }, form: 0.04, tag: 'shirt-pinned' } },
        ],
      },
    },
  },
  {
    ...
    beats: {
      open: {
        ...
        choices: [
          { id: 'entertain', label: 'The one who makes it worth watching', desc: 'People remember how you made them feel', outcome: 'He decides he would rather be the reason somebody came than the reason somebody won, and half suspects those are the same thing anyway.', effect: { attr: { flair: 2, creativity: 1 }, form: 0.05, meters: { peers: 5 }, tag: 'wants-entertainer' }, next: 'test' },
          { id: 'reliable', label: 'The one you can depend on', desc: 'Same player in the rain as in the sun', outcome: 'He decides he wants to be the one nobody worries about, which is the least glamorous ambition available and the hardest to fake.', effect: { attr: { teamwork: 2, composure: 1 }, meters: { authority: 6 }, tag: 'wants-reliable' }, next: 'test' },
          { id: 'winner', label: 'The one who decides games', desc: 'Be there at the end, every time', outcome: 'He decides he wants to be the one it goes through when it matters. It is an ambition with teeth in it, and he will find out later exactly where they are.', effect: { attr: { leadership: 2, aggression: 1 }, form: 0.05, tag: 'wants-decider' }, next: 'test' },
        ],
      },
      test: {
        id: 'test',
        ...
        choices: [
          ...
          { id: 'adapt', label: 'Play what the day asks for', desc: 'Be the other kind of player for ninety minutes', outcome: 'He puts the idea in his pocket and does the ugly job well, and discovers that being able to switch is itself a style. Nobody claps. He knows.', effect: { attr: { composure: 2, teamwork: 1 }, meters: { authority: 6 }, form: 0.04 } },
        ],
      },
    },
  },
  ...
  },
  {
    ...
    beats: {
      open: {
        ...
        choices: [
          { id: 'feed', label: 'Feed it — carry a ball everywhere', desc: 'Never be more than a foot away from one', outcome: 'A tennis ball lives in his coat pocket for two years. The touch it builds is not the sort you can be coached into; it arrives through sheer accumulated fiddling.', effect: { attr: { flair: 2, creativity: 1 }, form: 0.06, meters: { school: -3 }, tag: 'always-a-ball' } },
          { id: 'contain', label: 'Keep it out of school', desc: 'Two confiscations is enough', outcome: 'He rations himself to home and the walk, which annoys him and saves him three detentions. The fiddling survives. It just gets better hours.', effect: { attr: { composure: 1, flair: 1 }, meters: { school: 5 } } },
          { id: 'share', label: 'Turn it into a game with whoever is nearby', desc: 'Keepy-up chains in the bus queue', outcome: 'It becomes a thing several of them do, badly, in doorways. He is the reason a small corner of the school is football-mad that winter.', effect: { attr: { teamwork: 1, flair: 1 }, meters: { peers: 6, school: -2 }, form: 0.04 } },
        ],
      },
    },
  },
    ...
    beats: {
      open: {
        ...
        choices: [
          ...
          { id: 'neither', label: 'Refuse to choose', desc: 'Take a bit from each and see', outcome: 'He decides the whole question is a false one invented by people who make highlight clips, and carries on being an unclassifiable twelve-year-old.', effect: { attr: { creativity: 2, composure: 1 }, tag: 'style-undecided' }, next: 'later' },
        ],
      },
      later: {
        id: 'later',
        ...
        choices: [
          ...
          { id: 'push', label: 'Set out to prove it wrong', desc: 'Be the other thing on purpose', outcome: 'He spends the whole spring deliberately being the player nobody said he was, and comes out the far side genuinely wider. Being underestimated is fuel if you can be bothered to burn it.', effect: { attr: { leadership: 2, flair: 1 }, energy: -5, form: 0.05 } },
        ],
      },
    },
  },
