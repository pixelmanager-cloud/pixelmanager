...
export const YOUTH_JOY_ARCS: StoryArc[] = [
  {
    ...
    beats: {
      ...
      use: {
        ...
        choices: [
          { id: 'try', label: 'Try it', desc: 'In a real game, with people watching', outcome: 'It comes off. Nobody on the touchline knows where it came from except him, and he does not tell anyone, ever.', effect: { form: 0.08, attr: { flair: 1 }, meters: { authority: -3, peers: 5 } } },
  ...
  },
  {
    ...
    beats: {
      open: {
        ...
        choices: [
          { id: 'idol', label: 'The player he loves', desc: 'SADIKU, 10 — the obvious one, and he does not care', outcome: 'SADIKU 10. He wears it until the collar goes soft, and years later he can still feel exactly which letters had started to peel.', effect: { meters: { family: -2 }, form: 0.05, attr: { flair: 1 }, tag: 'shirt-idol' } },
    ...
    beats: {
      open: {
        ...
        choices: [
          ...
          { id: 'four', label: 'Take the four', desc: 'Nobody grows up dreaming of the four', outcome: 'He takes the four because somebody has to, and finds he quite likes being the one the game goes through rather than the one it ends at.', effect: { attr: { teamwork: 2, composure: 1 }, meters: { peers: -2, authority: 5 }, tag: 'number-four' } },
          { id: 'odd', label: 'Take the twenty-three with the hole', desc: 'For no reason he can explain', outcome: 'He takes the broken one because he likes the number, and wears it for three seasons out of pure stubbornness until it means something simply by having lasted.', effect: { attr: { flair: 1, composure: 1 }, meters: { authority: -2, peers: 4 }, tag: 'number-23' } },
        ],
      },
    },
  },
    ...
    beats: {
      open: {
        ...
        choices: [
          ...
          { id: 'test', label: 'Deliberately break it once', desc: 'To prove he is not superstitious', outcome: 'He puts the right boot on first, on purpose, staring at it. He plays fine. He is slightly disappointed by how fine he plays.', effect: { meters: { family: -2 }, attr: { composure: 1, leadership: 1 }, tag: 'ritual-tested' }, next: 'broken' },
        ],
      },
      broken: {
        id: 'broken',
  ...
  },
  {
    ...
    beats: {
      open: {
        ...
        choices: [
          { id: 'ask', label: 'Ask him something real', desc: 'Not an autograph — a question', outcome: 'He asks how long it took him to learn the turn. Sadiku thinks about it properly and says two years, and that it still goes wrong. It is the most useful sentence anyone says to him that whole year.', effect: { meters: { peers: -2 }, attr: { composure: 2, creativity: 1 }, form: 0.05, tag: 'hero-spoke' }, next: 'after' },
    ...
    beats: {
      open: {
        ...
        choices: [
          ...
          { id: 'frame', label: 'Pin it to the bedroom wall', desc: 'Where he can see it from the bed', outcome: 'Four drawing pins and a slightly crooked hang. It is the first thing he sees every morning for two years, and it is not nostalgia — it is a to-do list.', effect: { meters: { peers: -2 }, attr: { leadership: 1, composure: 1 }, form: 0.04, tag: 'shirt-pinned' } },
        ],
      },
    },
  },
  {
    ...
    beats: {
      open: {
        ...
        choices: [
          { id: 'entertain', label: 'The one who makes it worth watching', desc: 'People remember how you made them feel', outcome: 'He decides he would rather be the reason somebody came than the reason somebody won, and half suspects those are the same thing anyway.', effect: { attr: { flair: 2, creativity: 1 }, form: 0.05, meters: { authority: -4, peers: 5 }, tag: 'wants-entertainer' }, next: 'test' },
          { id: 'reliable', label: 'The one you can depend on', desc: 'Same player in the rain as in the sun', outcome: 'He decides he wants to be the one nobody worries about, which is the least glamorous ambition available and the hardest to fake.', effect: { attr: { teamwork: 2, composure: 1 }, meters: { peers: -3, authority: 6 }, tag: 'wants-reliable' }, next: 'test' },
  ...
  },
  {
    ...
    beats: {
      open: {
        ...
        choices: [
          { id: 'imagine', label: 'Stand in the centre circle and fill it', desc: 'All of it, every seat, out loud in his head', outcome: 'He stands there long enough that a groundsman coughs. In the ninety seconds before that he has heard it full, and it does not frighten him. Worth knowing.', effect: { meters: { school: -2 }, form: 0.06, attr: { composure: 2, flair: 1 }, tag: 'imagined-crowd' } },
  ...
  },
  {
    ...
    beats: {
      open: {
        ...
        choices: [
          { id: 'plain', label: 'Choose the plain one', desc: 'Do the simple thing faster than everyone else', outcome: 'He picks Vennick and spends a season removing things from his game rather than adding them. It is much less fun and it works almost immediately.', effect: { attr: { composure: 2, teamwork: 1 }, form: 0.06, meters: { peers: -3, authority: 6 }, tag: 'style-plain' }, next: 'later' },
          { id: 'risky', label: 'Choose the risky one', desc: 'Be the reason something happens', outcome: 'He picks the other one, and accepts in advance that a lot of afternoons will end with him being the problem. Some will end with him being the only reason.', effect: { attr: { flair: 2, creativity: 1 }, form: 0.04, meters: { authority: -4, peers: 5 }, tag: 'style-risk' }, next: 'later' },
          { id: 'neither', label: 'Refuse to choose', desc: 'Take a bit from each and see', outcome: 'He decides the whole question is a false one invented by people who make highlight clips, and carries on being an unclassifiable twelve-year-old.', effect: { meters: { authority: -2, peers: -2 }, attr: { creativity: 2, composure: 1 }, tag: 'style-undecided' }, next: 'later' },
        ],
      },
      later: {
        id: 'later',
