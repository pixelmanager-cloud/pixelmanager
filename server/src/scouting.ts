...
const BAND_Q: Record<Band, [number, number]> = { raw: [6, 9], squad: [10, 12], quality: [13, 14], gem: [14, 15] };
...
function rollAt(accountId: string, seasonNumber: number, index: number, tier: string): { player: Player; band: Band } {
  ...
  for (const b of ['gem', 'quality', 'squad', 'raw'] as Band[]) { acc += dist[b]; if (bandRng < acc) { band = b; break; } }
  const [lo, hi] = BAND_Q[band];
  const q = lo + ((s % 997) / 997) * (hi - lo);
  return { player: generateTrialist(`loan-s${seasonNumber}-${index}`, q, s), band };
}
...
export function generatePool(accountId: string, seasonNumber: number, tier = 'base'): TrialInfo[] {
  return Array.from({ length: POOL_SIZE }, (_, i) => {
    const { player, band } = rollAt(accountId, seasonNumber, i, tier);
    return { index: i, id: player.id, name: player.name, role: player.role, overall: overall(player), band };
  });
}
