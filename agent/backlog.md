...
- [x] Turn the match "Your squad fitness: X%" readout (#fit-label in client/index.html, updated in client/src/main.ts syncMatchHud) into a small horizontal bar that fills to the fitness % and shifts colour green→amber→red as it drops, keeping the % text. Client + CSS + minimal main.ts change only; no dependencies.
