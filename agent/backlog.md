...
- [ ] **Polish the MATCH-VIEW visuals** so matches feel more alive and readable. CLIENT-ONLY and DETERMINISTIC: work in `client/src` (mainly the Phaser match scene in `main.ts` + `pixelart.ts`); make NO changes to `shared/` (the engine) and do NOT alter game balance — player/ball positions still come straight from the engine state each tick. First read `client/src/main.ts` (the match scene + render loop), `client/src/pixelart.ts` (the pixel textures), and skim `shared/src/engine.ts` types so you know what state is available (positions, carrier, events, fitness, score, clock). Implement SEVERAL of: (1) **richer pitch** — centre circle + halfway line, both penalty boxes + 6-yard boxes + penalty spots, goal nets/mouths, corner arcs, and subtle mown-grass stripes, all in the existing retro palette; (2) **better players** — face the direction they're moving, a subtle bob/run cadence, clearly highlight the ball-carrier (glow/outline), distinct team kit colours, optional shirt numbers; (3) **ball** — a drop shadow + a smoother fading trail + a little spin; (4) **event flourishes driven by the existing engine events** (goal/shot_saved/shot_missed/chance) — a shot streak line, a keeper save pop, and a bigger "GOAL!" celebration that names the scorer; (5) a **subtle camera** ease/zoom toward the action (never disorienting). Keep it 60fps-smooth and framework-free of new deps. Purely-cosmetic client render jitter (e.g. `Math.random()` for a sparkle) is allowed in the CLIENT, but NEVER in `shared/`. Open a PR; `npm run verify` must pass. If you can't fit everything in one turn, ship a coherent subset and list the rest in the PR body.
